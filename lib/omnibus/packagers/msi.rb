...
module Omnibus
  class Packager::MSI < Packager::WindowsBase
    ...
    #
    # Get the shell command to create a zip file that contains
    # the unstripped binary files. Those files will be deleted
    # after the zip.
    #
    # @return [String]
    #
    def zip_unstripped_files_command
      <<-EOH.split.join(" ").squeeze(" ").strip
      7z a -r -sdel
      #{windows_safe_path(Config.package_dir)}\\#{project.name + ".debug"}.zip
      #{windows_safe_path(File.join(project.install_dir, debug_package_paths))}\\*
      EOH
    end
